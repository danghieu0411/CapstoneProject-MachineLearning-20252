...
*   **HOG + Support Vector Machine (SVM)**: 
    ...
    *   **Preprocessing**: Resized to 64 × 64 pixels and converted to grayscale. 
...
*   **SIFT + BoVW + Random Forest (RF)**: 
    *   **Dataset Size**: A representative subset of **4,000 images** (2,000 cats and 2,000 dogs) was randomly sampled.
    *   **Preprocessing**: Resized to 256 × 256 pixels and converted to grayscale.
...
*   **Artificial Neural Network (ANN)**:
    *   **Preprocessing**: Resized to 64 × 64 pixels with 3-channel RGB colors, flattened into a 12,288-dimensional input vector.
    *   **Data Split**: Partitioned into **20,000 images** for training, **2,500 images** for validation, and **2,500 images** for testing.
*   **CNN from Scratch**:
    *   **Preprocessing**: Resized to 224 × 224 pixels, 3-channel RGB. Data augmentation (Random Resized Crop, Horizontal Flip, Color Jitter) and ImageNet normalization were applied.
    *   **Data Split**: Partitioned into **20,000 images** for training, **2,500 images** for validation, and **2,500 images** for testing.
*   **Residual Network (ResNet-18)**:
    *   **Preprocessing**: Resized to 224 × 224 pixels, 3-channel RGB.
    *   **Data Split**: Partitioned into **20,000 images** for training, **2,500 images** for validation, and **2,500 images** for testing.
...
1.  **Image Preprocessing**: Input RGB images are converted to grayscale to focus strictly on shape contours rather than color variance. They are then standardized to a spatial resolution of 64 × 64 pixels to ensure a constant vector dimensionality.

![HOG Extraction Pipeline](report/hog_pipeline.png)
2.  **Gradient Calculation**: For each pixel, the horizontal gradient *G_x* and vertical gradient *G_y* are computed by filtering the image with 1D Sobel-like kernels: *K_x* = [-1, 0, 1] and *K_y* = [-1, 0, 1]ᵀ.
    The gradient magnitude *G*(*x*, *y*) and orientation *θ*(*x*, *y*) are defined as:
    > **G(x, y) = √(G_x(x, y)² + G_y(x, y)²)**
    >
    > **θ(x, y) = arctan(G_y(x, y) / G_x(x, y))**
    
    The angle *θ* is unsigned, wrapping within the range [0°, 180°] to ensure invariance to sign changes in lighting.

![Gradient Magnitudes and Directions](report/hog_gradients.png)

3.  **Spatial Cell Partitioning and Orientation Binning**: The image is divided into contiguous, non-overlapping cells of 8 × 8 pixels. For each cell, a 9-bin orientation histogram is created, where bins correspond to orientations of 0°, 20°, 40°, ..., 160°.

![Cells and Blocks](report/hog_cells_blocks.png)
    Each pixel inside the cell casts a vote into the histogram. The vote's bin is determined by the pixel's gradient angle *θ*, and its weight is determined by the gradient magnitude *G*. To avoid quantization noise, votes are split proportionally between adjacent orientation bins.
4.  **Block Normalization**: Gradient strengths vary extensively due to lighting and local illumination changes. To make the descriptor robust to contrast variations, local histograms are normalized over larger, overlapping spatial blocks of 2 × 2 cells (which equals 16 × 16 pixels).
    The histograms of the 4 cells in a block are concatenated into a 36-dimensional vector *v*. The vector is then normalized using the L₂-norm:
    > **v_normalized = v / √(||v||₂² + ε²)**
    
    where *ε* = 10⁻⁵ is a small stabilization constant to prevent division by zero.
5.  **Final Feature Vector Construction**: The blocks slide across the image with a step size (stride) of 8 pixels. For a 64 × 64 pixel image:
    *   Horizontal blocks: (64 - 16) / 8 + 1 = 7 blocks.
    *   Vertical blocks: (64 - 16) / 8 + 1 = 7 blocks.
    *   Total blocks: 7 × 7 = 49 blocks.
    
    Concatenating the 36 features of all 49 blocks yields the final HOG descriptor vector:
    > **49 blocks × 36 features/block = 1764 dimensions**
...
1.  **Optimization Objective**: Given a training set of HOG features *X_i* ∈ ℝ¹⁷⁶⁴ and binary class labels *y_i* ∈ {-1, 1} (where Cat = -1, Dog = 1), the model learns weights *w* ∈ ℝ¹⁷⁶⁴ and bias *b* ∈ ℝ by minimizing Hinge Loss combined with L2 weight regularization:
    > **min_{w, b} L(w, b) = (1/2) ||w||² + (C/N) ∑_{i=1..N} max(0, 1 - y_i(wᵀ X_i - b))**
    where:
    *   ||*w*||² is the regularization term maximizing the geometric margin.
...
2.  **Stochastic Gradient Descent (SGD) Parameter Update**: The global minimum of the loss objective is searched iteratively using SGD. For a single sample *x_i*, the gradients are derived based on the Hinge condition:
    *   **Case 1**: If the sample is classified correctly and lies outside the margin boundary (*y_i*(*w*ᵀ *x_i* - *b*) ≥ 1):
        > **∂L_i / ∂w = w** and **∂L_i / ∂b = 0**
    *   **Case 2**: If the sample violates the margin or is misclassified (*y_i*(*w*ᵀ *x_i* - *b*) < 1):
        > **∂L_i / ∂w = w - C * y_i * x_i** and **∂L_i / ∂b = C * y_i**
    
    The parameters are updated at each step with learning rate *η*:
    > **w = w - η * (∂L_i / ∂w)**
    >
    > **b = b - η * (∂L_i / ∂b)**
...
##### A. Hyperparameters
*   **Input Image Resolution**: 64 × 64 pixels (Grayscale).
...
*   **SVM Configuration**:
    ...
    *   Regularization parameter (*λ*): 0.01 (controls weight penalty).
...
The SGD successfully converged without showing severe oscillations, showing that the learning rate *η* = 0.0001 combined with the weight decay *λ* = 0.01 was appropriate.
...
1.  **Scale-Space Extrema Detection**: The image is convolved with Gaussian filters at different scales, and the Difference of Gaussians (DoG) is calculated to identify potential keypoints that are invariant to scale and orientation:
    ...
    where *L*(*x*, *y*, *σ*) is the scale-space representation of the image convolved with a Gaussian of standard deviation *σ*.
...
4.  **Keypoint Descriptor Generation**: A 16 × 16 pixel neighborhood around the keypoint is divided into sixteen 4 × 4 sub-regions. For each sub-region, an 8-bin gradient orientation histogram is computed. Concatenating these histograms yields a feature descriptor vector of dimension:
    > **16 sub-regions × 8 bins = 128 dimensions**
...
3.  **L2-Normalization**: To prevent images with a high density of keypoints from biasing the classifier, the frequency histogram *h* is L2-normalized:
    ...
    where *ε* = 10⁻⁷ is a smoothing factor. This yields a fixed-length, normalized vector *x* ∈ ℝ⁵⁰⁰ representing the global distribution of local textures in the image.
...
2.  **Random Feature Selection**: To reduce correlation between trees and improve generalization, at each split node in a decision tree, only a random subset of the 500 features is evaluated for the best split:
    > **m = √k = √500 ≈ 22 features**
3.  **Classification Voting**: For a test image represented by its BoVW histogram *x* ∈ ℝ⁵⁰⁰, each of the 100 decision trees outputs a binary vote (0 for Cat, 1 for Dog). The final prediction corresponds to the majority vote of the ensemble.
...
*   **Memory Footprint**: Highly compact. The model only needs to store the K-Means centroids (500 × 128 floats) and the 100 decision trees, which occupy very little RAM compared to deep learning models.
...
2.  **Parameter Explosion & Overfitting**: Because every input node connects to every neuron in the subsequent layer, the number of parameters grows rapidly. A Dense layer with 256 neurons processing a 12,288-dimensional vector requires:
    > **12,288 × 256 + 256 = 3,146,240 parameters**
    Such a high parameter footprint makes the model highly susceptible to overfitting, especially on limited datasets.

To mitigate these limits, techniques such as **Data Augmentation** (random horizontal flips, rotations, and zooms), **L₂ Regularization**, and **Dropout** are incorporated into the training pipeline.
...
> **P(y = i | X) = e^(z_i) / ∑_{j=1..K} e^(z_j)**
Thus, the individual class probabilities are computed as:
> **P(Cat) = e^(z_cat) / (e^(z_cat) + e^(z_dog))**
>
> **P(Dog) = e^(z_dog) / (e^(z_cat) + e^(z_dog))**
The final predicted class corresponds to the index with the maximum probability:
> **ŷ = argmax(P(Cat), P(Dog))**

##### C. Regularization and Dropout
To control overfitting, the model minimizes a regularized loss function consisting of Sparse Categorical Cross-Entropy and an L₂ weight penalty:
> **L_total = - (1/N) ∑_{i=1..N} log P(y_i | X_i) + λ ∑_w w²**
Where:
*   *N* is the number of training samples.
*   *y_i* is the true label of sample *i*.
*   *λ* = 0.0005 is the regularization coefficient (weight decay).
...
*   **Regularization**: L2 regularization on the hidden layer weights with *λ* = 0.0005.
...
*   **Optimizer**: Adam with a learning rate of *α* = 0.0001.
*   **Loss Function**: Sparse Categorical Cross-Entropy.
*   **Evaluation Metric**: Accuracy.
*   **Batch Size**: 32.
*   **Epochs**: 30.
*   **Callbacks**:
    *   `ReduceLROnPlateau`: Monitors validation loss (`val_loss`). If the validation loss does not improve for 2 consecutive epochs, the learning rate is halved (*α*_new = 0.5 × *α*_old), with a minimum learning rate limit of 10⁻⁶.
...
*   At epoch 12, `ReduceLROnPlateau` reduced the learning rate to 5.00 × 10⁻⁵ as validation loss stabilized. It was reduced again to 2.50 × 10⁻⁵ at epoch 19, and to 1.25 × 10⁻⁵ at epoch 22.
...
##### B. Model Quality and Generalization
*   Although regularizations like L2 weight decay (*λ* = 0.0005) and Dropout (*p* = 0.4) successfully prevented severe overfitting (as training and validation metrics tracked closely), the final classification accuracy remains low (64.48%).
...
1.  **Loss of Spatial Structure**: Flattening a 2D image *H* × *W* × *C* into a 1D vector destroys the local spatial layout. The relationships between neighboring pixels, which form edges, corners, and texture patterns, are lost.
2.  **Parameter Explosion**: In a fully connected layer, every input pixel connects to every hidden neuron. For a modest 224 × 224 × 3 image (150,528 dimensions), connecting to a hidden layer of only 512 neurons requires:
    > **150,528 × 512 + 512 ≈ 77 million parameters**
...
-   **Pooling (Max Pooling)**: Performs spatial downsampling by extracting the maximum value within a window (e.g., 2 × 2 window with stride 2). This reduces spatial dimensions by 75%, controls overfitting, and grants translation invariance.
-   **Global Average Pooling (GAP)**: Instead of flattening the final 3D feature map into a massive 1D vector (which introduces millions of parameters), GAP computes the spatial average of each feature map channel (*H* × *W* × *C* → 1 × 1 × *C*). This makes the transition to fully connected layers parameter-free and acts as a strong regularizer.
...
*   **Channel Progression**: 3 → 32 → 64 → 128 → 256 channels
*   **Convolutional Blocks**: Each block features 2 stacked Conv2d layers (3 × 3 kernel, stride 1, padding 1), Batch Normalization, ReLU activation, and a MaxPool2d layer (2 × 2 window, stride 2).
*   **Global Average Pooling (GAP)**: Downsamples the final 14 × 14 × 256 feature map to a 1 × 1 × 256 vector.
...
##### A. Training Configuration and Callbacks
*   **Optimizer**: Adam with learning rate *α* = 0.001 and weight decay of 10⁻⁴ (L2 regularization).
*   **Loss Function**: Cross-Entropy Loss.
*   **Batch Size**: 64.
*   **Epochs**: 25.
*   **Scheduler**: `ReduceLROnPlateau` configured to halve the learning rate (factor = 0.5) if the validation loss does not improve for 2 consecutive epochs.
*   **Data Augmentation**: To prevent overfitting, training images were resized to 256 × 256, cropped randomly to 224 × 224, horizontally flipped (*p*=0.5), rotated randomly (±15°), and color jittered (brightness and contrast).
...
*   The learning rate was reduced by the scheduler at epoch 14 to 3 × 10⁻⁵ as validation loss stabilized, leading to tighter convergence.
...
2.  **Unhindered Gradient Propagation**: During backpropagation, the derivative of the output *y* = *F*(*x*) + *x* with respect to the input *x* is:
    > **∂L/∂x = (∂L/∂y) * (∂y/∂x) = (∂L/∂y) * (∂F(x)/∂x + 1) = (∂L/∂y) * (∂F(x)/∂x) + ∂L/∂y**
    This shows that the gradient ∂L/∂y is added directly to the backpropagated error. Even if the weight pathways in the convolutional branch ∂F(x)/∂x vanish to zero, the gradient still flows through the shortcut pathway (the term + ∂L/∂y), ensuring early layers are successfully updated regardless of depth.
...
##### A. Hyperparameters
*   **Input Resolution**: 224 × 224 × 3 (RGB)
*   **Stem Layer (Layer 0)**: Conv 7 × 7 (stride 2), Batch Normalization, ReLU, MaxPool 3 × 3 (stride 2). Input size is downsampled from 224 × 224 × 3 to 56 × 56 × 64.
*   **Residual Stages**:
    *   **Layer 1**: 2 blocks of stacked Conv 3 × 3 (64 → 64 channels). Shortcut: Identity mapping.
    *   **Layer 2**: 2 blocks of stacked Conv 3 × 3 (64 → 128 channels). Shortcut: Downsampled via Conv 1 × 1 with stride 2.
    *   **Layer 3**: 2 blocks of stacked Conv 3 × 3 (128 → 256 channels). Shortcut: Downsampled via Conv 1 × 1 with stride 2.
    *   **Layer 4**: 2 blocks of stacked Conv 3 × 3 (256 → 512 channels). Shortcut: Downsampled via Conv 1 × 1 with stride 2.
*   **Output Block**: Global Average Pooling (7 × 7 → 1 × 1), Flattening, Dropout (*p*=0.3), and a Linear classifier mapping 512 channels to 2 output classes.
...
| :--- | :--- | :---: | :---: | :---: | :---: | :---: |
| **Layer 0 (Stem)** | Conv 7×7 (s=2) → MaxPool 3×3 (s=2) | 224 × 224 | 56 × 56 | 3 | 64 | No |
| **Layer 1** | 2 × BasicBlock (2 × Conv 3 × 3) | 56 × 56 | 56 × 56 | 64 | 64 | No (Identity) |
| **Layer 2** | 2 × BasicBlock (2 × Conv 3 × 3) | 56 × 56 | 28 × 28 | 64 | 128 | Yes (Conv 1×1) |
| **Layer 3** | 2 × BasicBlock (2 × Conv 3 × 3) | 28 × 28 | 14 × 14 | 128 | 256 | Yes (Conv 1×1) |
| **Layer 4** | 2 × BasicBlock (2 × Conv 3 × 3) | 14 × 14 | 7 × 7 | 256 | 512 | Yes (Conv 1×1) |
| **Output Block**| GAP → Flatten → FC Layer | 7 × 7 | Vector | 512 | 2 (num_classes) | - |

#### 5.3. Training Process

##### A. Training Configuration
*   **Optimizer**: Adam with learning rate *α* = 0.0003.
*   **Loss Function**: Cross-Entropy Loss.
*   **Batch Size**: 64.
*   **Epochs**: 20.
...
*   **Augmentation Pipeline**: Train: resize (256 × 256), crop (224 × 224), horizontal flip, rotation (±15°), and color jitter (brightness, contrast). Validation/Test: resize (224 × 224) and normalization.
...
2.  **Hardware Restrictions**: Expanding GPU memory constraints would allow training larger batch sizes and higher-resolution inputs (384 × 384) to extract more fine-grained features.
